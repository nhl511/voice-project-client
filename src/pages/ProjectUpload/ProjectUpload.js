...
const ProjectUpload = () => {
  ...
  const uploadProjectURL = '/api/Buyers/UploadVoiceProject/{BuyerId},{title},{description},{price},{duration},{numberOfEdit},{deadline}';
  const { auth } = useAuth();
  ...
  const handleUploadFileDemo = async (e) => {
    ...
    try {
      await axios.post(uploadFileDocURL, fileDemo, { headers })
        .then((response) => {
  ...
  const handleUploadFileMain = async (e) => {
    ...
    try {
      await axios.post(uploadFileDocURL, fileMain, { headers })
        .then((response) => {
          if (response.status === 200) {
  ...
  const handleUploadThumbnail = async (e) => {
    ...
    try {
      await axios.post(uploadThumbnailURL, fileThumbnail, { headers })
        .then((response) => {
          if (response.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`/api/Buyers/UploadVoiceProject/${auth.userId},${title},${description},${price},${duration},${numberOfEdit},${deadline}`,
        infoProject, { headers })
        .then((response) => {
          if (response.status === 200) {
          ...
          <div>
            <form onSubmit={handleSubmit}>
              <div className="projectUpload-card-first">
                <input placeholder="Tiêu đề" required onChange={(e) => setTitle(e.target.value)} />
                <input placeholder="Mô tả" required onChange={(e) => setDescription(e.target.value)} />
                <input placeholder="Yêu cầu" onChange={(e) => setRequest(e.target.value)} />
              </div>
              <div className="projectUpload-card-second">
                <div className="projectUpload-card-grid">
                  <div className="projectUpload-col">
                    ...
                    <input type="file" id="btn-upload-voice" hidden accept=".doc, .docx, .pdf, .txt" required
                      onChange={handleUploadFileDemo} />
                    <label htmlFor="btn-upload-voice">{nameFileDemo ? nameFileDemo : 'Upload'}</label>
                  </div>
                  <div className="projectUpload-col">
                    <span>Văn bản cần đọc:</span>
                    <input type="file" id="btn-upload-doc" hidden accept=".doc, .docx, .pdf, .txt" required
                      onChange={handleUploadFileMain} />
                    <label htmlFor="btn-upload-doc">{nameFileMain ? nameFileMain : 'Upload'}</label>
                  </div>
                </div>
                <div className="projectUpload-card-display">
                  ...
                  <div className="projectUpload-first-row-col-3">
                    <span>Thumbnail:</span>
                    <input type="file" id="thumbnail" hidden accept=".png, .jpg, .jpeg"
                      onChange={handleUploadThumbnail} />
                    <label htmlFor="thumbnail">{nameFileThumbnail ? nameFileThumbnail : 'Upload'}</label>
                  </div>
                  ...
                  <div className="projectUpload-fifth-row-col-1">
                    ...
                    <input type="number" onChange={(e) => setNumberOfEdit(e.target.value)} />
                  </div>
                  <div className="projectUpload-fifth-row-col-2">
                    <span>Độ dài văn bản:</span>
                    <input type="number" onChange={(e) => setTextLength(e.target.value)} />
                  </div>
