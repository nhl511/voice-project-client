...
const VoiceProfile = () => {
  ...
  const handleUploadVoice = async (event) => {
    ...
    try {
      await axios.post(uploadVoiceURL, file, { headers })
        .then((response) => {
  ...
  const handleUploadProfile = async (e) => {
    ...
    try {
      await axios.post(uploadVoiceProfileURL, uploadProfile, { headers })
        .then((response) => {
          if (response.status === 200) {
        ...
        .catch((error) => {
          console.log(error); alert('upload failed')
        })
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="voiceprofile-card">
            <div className="col-item">
              <span className="sub-title">
                Để hoàn tất profile bạn vui lòng đọc văn bản dưới đây và gửi bản ghi âm giọng của bạn lên hệ thống.
              </span>
              <Link to="/mp3/test.docx" download="Test-Docx-Document" target="blank" >
            ...
            <form onSubmit={handleUploadProfile}>
              ...
              <div className="col-item">
                <input type="file" name="file" id="upload-btn" onChange={handleUploadVoice} hidden accept="audio/mpeg" />
